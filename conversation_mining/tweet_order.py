...
def main():
    ...
    if trees:
        ...
        # Store the processed trees in the valid_trees collection
        serialized_trees = [tree.to_json(with_data=True) for tree in trees]
        deserialized_trees = [json.loads(tree) for tree in serialized_trees]
        valid_trees_collection.insert_many(deserialized_trees)
        print(f"Inserted {len(deserialized_trees)} trees into 'valid_trees' collection.")
    else:
        print("No valid trees found matching the defined order criteria.")
